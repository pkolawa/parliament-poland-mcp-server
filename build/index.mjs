...
// src/utils/api.ts
var SEJM_API_BASE = "https://api.sejm.gov.pl/sejm";
async function makeSejmRequest(endpoint) {
...
var getTermsTool = {
  ...
  schema: {},
  handler: async () => {
    const terms = await makeSejmRequest("/term");
    if (!terms) {
      return {
        content: [
          {
...
var getMpTool = {
  ...
  handler: async (args) => {
    const { term, id } = args;
    const mp = await makeSejmRequest(`/term${term}/MP/${id}`);
    if (!mp) {
      return {
        content: [
          {
...
var getMpsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const mps = await makeSejmRequest(`/term${term}/MP`);
    if (!mps) {
      return {
        content: [
          {
...
var getClubsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const clubs = await makeSejmRequest(`/term${term}/clubs`);
    if (!clubs) {
      return {
        content: [
          {
...
// src/tools/committees.ts
var getCommitteesTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const committees = await makeSejmRequest(`/term${term}/committees`);
    if (!committees) {
      return {
        content: [
          {
...
// src/tools/interpellations.ts
var getInterpellationsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const interpellations = await makeSejmRequest(`/term${term}/interpellations`);
    if (!interpellations) {
      return {
        content: [
          {
...
var getPrintsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const prints = await makeSejmRequest(`/term${term}/prints`);
    if (!prints) {
      return {
        content: [
          {
...
// src/tools/proceedings.ts
var getProceedingsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const proceedings = await makeSejmRequest(`/term${term}/proceedings`);
    if (!proceedings) {
      return {
        content: [
          {
...
var getProcessesTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const processes = await makeSejmRequest(`/term${term}/processes`);
    if (!processes) {
      return {
        content: [
          {
...
var getTranscriptsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const transcripts = await makeSejmRequest(`/term${term}/transcripts`);
    if (!transcripts) {
      return {
        content: [
          {
...
var getVideosTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const videos = await makeSejmRequest(`/term${term}/videos`);
    if (!videos) {
      return {
        content: [
          {
...
var getVotingsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const votings = await makeSejmRequest(`/term${term}/votings`);
    if (!votings) {
      return {
        content: [
          {
...
var getWrittenQuestionsTool = {
  ...
  handler: async (args) => {
    const { term } = args;
    const writtenQuestions = await makeSejmRequest(`/term${term}/written-questions`);
    if (!writtenQuestions) {
      return {
        content: [
          {
